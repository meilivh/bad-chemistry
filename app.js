...
function drawWaffleChart(fullWidth, fullHeight, chart) {
    ...
    const DOT_GAP = 3;
    // dot radius computed from available width so the grid scales with screen size
    const DOT_R = Math.max(4, Math.floor(((outerWidth - BAR_W - BAR_GAP) / COLS - DOT_GAP) / 2));
    ...
    const top50SpacingY = DOT_R_BIG * 2 ;
    const top50OffsetY = 20;

    const radiusScale = d3.scaleSqrt()
        .domain([d3.min(TOP50, d => d.fa), d3.max(TOP50, d => d.fa)])
